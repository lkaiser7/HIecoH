...
# same processing as 2_runoff_prob.R without separation by HIGAP type
# single class calculations for each landcover types across the state
# probability of landscape runoff given a single landcover class type
...
# head(kfs_class); table(kfs_class$cover_type); summary(kfs_class$Kfs)
# # remove negative values if any
# kfs_class<-kfs_class[which(kfs_class$Kfs >= 0),]

# pdf points by HIGAP class
pdf_class<-read_csv("outputs/AllRasters_coords_pdf_higap.csv")
...
# create an empty raster object for pdf points
...
for (h in 1:4){  # set h = 3 for debugging
  # select higap class
  c_num<-h_num[h]
  c_name<-h_name[h]
  print(c_name)
  ...
  # # NOT DONE BY CLASS - ASSUMES UNIFORM LANDCOVER ACROSS THE LANDSCAPE 
  ...
  # use all pdf location points for each higap class
  loc<-pdf_hist[, 1:7]
  all_hist<-pdf_hist[, -1:-7]
  all_pgw<-pdf_pgw[, -1:-7]
  # head(rf_hist[1:3])
  # head(rf_pgw[1:3])

  # loop through each row and calculate ALL runoff probability
  for(r in 1:dim(all_hist)[1]){  # set r = 1 for debugging
  # for(r in 1:10){  # set r = 1 for debugging
    print(r)
    rf_dist_hist<-as.matrix(all_hist[r,])
    # hist(rf_dist_hist)
    rf_dist_pgw<-as.matrix(all_pgw[r,])
    # hist(rf_dist_pgw)
    r # for counting 
    ...
    if(r == 1){
      single_runoff<-prob_runoff
    }else{
      single_runoff<-rbind(single_runoff, prob_runoff)
    }
    
  }
  
  # save cumulative probability of runoff per higap class
  write_csv(single_runoff, paste0(outDir, "probabilities/ALL_", c_name, "_prob_runoff.csv"))
  
  # combine data by rows
  prob_90m<-merge(higap_spdf, single_runoff, by.x = "PDF_ROWS", by.y = "...1", all.x = T)
  # dim(higap_spdf); dim(prob_90m); head(prob_90m)
  # save master data file
  write_csv(prob_90m, paste0(outDir, "probabilities/ALL_", c_name, "_prob_runoff_90m.csv"))
  ...
  # rasterize your irregular points (not gridded)
  all_hist_rast<-rasterize(rast_df, rast, rast_df$HIST_RUNOFF_PROB, fun = mean)
  all_pgw_rast<-rasterize(rast_df, rast, rast_df$PGW_RUNOFF_PROB, fun = mean)
  # plot(mask(all_hist_rast, higap_main))
  # plot(mask(all_pgw_rast, higap_main))
  
  # save raster outputs masked to higap to remove edges not on land
  writeRaster(mask(all_hist_rast, higap_main), format = "GTiff", overwrite = TRUE,
              paste0("outputs/prob_rasters/single_class/ALL_", 
                     c_name,"_hist_prob_runoff_90m.tif"))
  writeRaster(mask(all_pgw_rast, higap_main), format = "GTiff", overwrite = TRUE,
              paste0("outputs/prob_rasters/single_class/ALL_", 
                     c_name,"_pgw_prob_runoff_90m.tif"))
}
